...
def gram_matrix(features, normalize=True):
    ...
    gram = torch.bmm(features.view(N, C, -1), features.permute(0, 2, 3, 1).view(N, -1, C))
    if normalize:
        gram = gram / (H * W * C)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, layer in enumerate(style_layers):
        ...
        style_loss += style_weights[i] * torch.sum((feat_gram - style_targets[i])**2)
...
def tv_loss(img, tv_weight):
    ...
    h_loss = torch.sum((img[:,:,:-1,:] - img[:,:,1:,:])**2)
    w_loss = torch.sum((img[:,:,:,:-1] - img[:,:,:,1:])**2)
